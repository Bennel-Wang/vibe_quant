...
from .utils.ohlcv import resample_to_weekly, resample_to_monthly
from .notification import notification_manager, PushPlusNotifier
from .scheduler import scheduler
from .scoring import scoring_core as _scoring_core, score_classification, compute_stock_score

# ─── Web Basic Auth ────────────────────────────────────────────────────────────
_web_cfg = config.get_web_config()
_AUTH_ENABLED = _web_cfg.get('auth_enabled', False)
_AUTH_USERNAME = _web_cfg.get('username', 'admin')
_AUTH_PASSWORD = _web_cfg.get('password', '')


def _check_auth(username: str, password: str) -> bool:
    return username == _AUTH_USERNAME and password == _AUTH_PASSWORD and bool(_AUTH_PASSWORD)


def _auth_required(f):
    """Decorator: HTTP Basic Auth（仅在 auth_enabled=true 时生效）"""
    @functools.wraps(f)
    def decorated(*args, **kwargs):
        ...
        if not auth or not _check_auth(auth.username, auth.password):
            return Response(
                '认证失败，请提供正确的用户名和密码。',
                401,
                {'WWW-Authenticate': 'Basic realm="Quant System"'}
            )
        return f(*args, **kwargs)
    return decorated
...
_BACKTEST_DIR = os.path.join(config.get('data_storage.backtest_dir', './data/backtests'))
os.makedirs(_BACKTEST_DIR, exist_ok=True)

logger = logging.getLogger(__name__)

# 后台回测进度（内存缓存，仅存储运行中状态；结果通过文件持久化）
BACKTEST_PROGRESS: Dict[str, Dict] = {}
BACKTEST_RESULTS: Dict[str, Dict] = {}  # 运行时内存，同时写文件
...
def _save_backtest_result(run_id: str, result: Dict):
    """将回测结果持久化到文件"""
    path = os.path.join(_BACKTEST_DIR, f"bt_{run_id}.json")
    try:
        with open(path, 'w', encoding='utf-8') as f:
            json.dump(result, f, ensure_ascii=False, indent=2, default=str)
    except Exception as e:
        logger.warning(f"保存回测结果失败 {run_id}: {e}")


def _load_backtest_result(run_id: str) -> Dict:
    """从文件加载回测结果"""
    path = os.path.join(_BACKTEST_DIR, f"bt_{run_id}.json")
    ...
    try:
        ...
        logger.warning(f"加载回测结果失败 {run_id}: {e}")
        return {}


def _list_backtest_results(limit: int = 50) -> list:
    """列出历史回测结果摘要（最新 limit 条）"""
    results = []
    try:
        files = sorted(
            [f for f in os.listdir(_BACKTEST_DIR) if f.startswith('bt_') and f.endswith('.json')],
            key=lambda x: os.path.getmtime(os.path.join(_BACKTEST_DIR, x)),
            reverse=True
        )[:limit]
        for fname in files:
            run_id = fname[3:-5]  # strip 'bt_' prefix and '.json' suffix
            r = _load_backtest_result(run_id)
            if r:
                results.append({
                    'run_id': run_id,
                    'code': r.get('code', ''),
...
# ── 策略匹配缓存（后台刷新，避免阻塞请求） ──
_strategy_match_cache: dict = {}
_strategy_match_ts: float = 0.0
_STRATEGY_MATCH_TTL = 900   # 15 分钟
_strategy_match_running: bool = False


def _scoring_core(df_ind) -> dict:
    """向后兼容别名 — 实现已移至 scoring.py"""
    from .scoring import scoring_core
    return scoring_core(df_ind)


def compute_stock_score(code: str) -> dict:
    """向后兼容别名 — 实现已移至 scoring.py"""
    from .scoring import compute_stock_score as _csc
    return _csc(code)
...
def api_strategy_match():
    """获取所有股票的策略匹配推荐（带后台缓存，避免阻塞）"""
    import time, threading
    global _strategy_match_cache, _strategy_match_ts, _strategy_match_running

    date = request.args.get('date')

    # 指定日期查询不走缓存，直接计算（历史分析用，用户等待可接受）
    if date:
        try:
    ...
    # 无 date 参数：走后台缓存
    now = time.time()
    if now - _strategy_match_ts < _STRATEGY_MATCH_TTL and _strategy_match_cache:
        return jsonify({'success': True, 'data': _strategy_match_cache})

    # 缓存过期/冷启动：触发后台刷新（不阻塞请求）
    if not _strategy_match_running:
        _strategy_match_running = True

        def _do_refresh():
            global _strategy_match_cache, _strategy_match_ts, _strategy_match_running
            try:
                # 先确保 scoring 缓存已预热（复用 get_all_scores() 触发的缓存）
                get_all_scores()
                from .strategy_matcher import strategy_matcher
                result = strategy_matcher.analyze_all_stocks()
                _strategy_match_cache = result
                _strategy_match_ts = time.time()
                logger.info("策略匹配缓存已刷新")
            except Exception as e:
                logger.error(f"策略匹配后台刷新失败: {e}", exc_info=True)
            finally:
                _strategy_match_running = False

        threading.Thread(target=_do_refresh, daemon=True).start()

    # 立即返回：有旧缓存则返回旧数据，否则返回 loading 状态
    if _strategy_match_cache:
        return jsonify({'success': True, 'data': _strategy_match_cache, 'refreshing': True})

    # 冷启动：返回大盘信息 + 空股票列表 + loading 标志
    try:
        from .market_regime import market_regime_detector
        market = market_regime_detector.detect()
        return jsonify({
            'success': True,
            'data': {'market': market.to_dict(), 'stocks': [], 'loading': True},
        })
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
...
def _get_effective_position_ratio(strategy) -> float:
    """返回策略的有效仓位比例（用于将总资金收益换算为仓位效率）.
    
    使用 max_position_ratio（策略最大总仓位）作为归一化基准，
    而非单笔 position_ratio（可能只有 4%，导致年化被放大 25 倍）。
    仅当 max_position_ratio >= 1.0（无上限）时才退回到单笔比例。
    """
    max_pos = strategy.max_position_ratio
    if max_pos < 1.0:
        return max_pos   # 用总仓位上限做归一化，如 0.6 → 最多部署 60% 资金
    # max_pos >= 1.0 时策略无明确上限，退回到单笔最大仓位
...
@app.route('/api/factors/best/export')
def api_factors_best_export():
    """导出最佳因子报告为 HTML 文件"""
    ...
    stock_results = res.get('stock_results', [])
    ...
    def _fmt(v, suffix='%', nd=2):
        if v is None:
            return '-'
        return f"{v:+.{nd}f}{suffix}" if suffix == '%' else f"{v:.{nd}f}"
    ...
    for entry in stock_results:
        bs = entry.get('best_strategy', {})
        pa = entry.get('position_advice') or {}
        score  = bs.get('composite_score')
        sc     = _score_color(score)
        real_a = bs.get('annual_return')        # 实际年化（与直接回测一致）
        adj_a  = bs.get('adj_annual_return')    # 仓位效率年化（÷ max_position_ratio）
        ra_color = '#28a745' if (real_a or 0) >= 0 else '#dc3545'
        rows_html += f"""
        <tr>
          <td><b>{entry.get('code','')}</b></td>
          <td>{entry.get('name','')}</td>
          <td style="max-width:200px;word-break:break-all">{bs.get('strategy_name','')}</td>
          <td style="color:{sc};font-weight:bold">{score if score is not None else '-'}</td>
          <td style="color:{ra_color};font-weight:bold">{_fmt(real_a)}</td>
          <td style="color:#1a73e8;font-size:0.9em">{_fmt(adj_a)}</td>
          <td>{_fmt(bs.get('win_rate'),'%',1)}</td>
          <td style="color:#dc3545">{_fmt(bs.get('max_drawdown_pct'),'%',1)}</td>
          <td>{_fmt(bs.get('total_return_pct'),'%',1)}</td>
          <td>{_fmt(bs.get('sharpe_ratio'),'',3)}</td>
          <td>{bs.get('total_trades','-')}</td>
          <td style="font-size:0.85em;color:#555">{pa.get('summary','') if pa else ''}</td>
        </tr>"""

    unfit_rows = ''
    for u in unfit_stocks:
        unfit_rows += f"<tr><td>{u.get('code','')}</td><td>{u.get('name','')}</td><td>{u.get('reason','')}</td></tr>"
...
<title>最佳因子挖掘报告 {now_str}</title>
<style>
  body{{font-family:'Microsoft YaHei',Arial,sans-serif;margin:0;padding:20px;background:#f8f9fa;color:#333}}
  .report-header{{background:linear-gradient(135deg,#1a73e8,#0d47a1);color:#fff;padding:24px 32px;border-radius:8px;margin-bottom:24px}}
  .report-header h1{{margin:0 0 6px;font-size:1.6em}}
  .report-header .meta{{opacity:.85;font-size:.9em}}
  .section{{background:#fff;border-radius:8px;box-shadow:0 1px 4px rgba(0,0,0,.08);margin-bottom:20px;overflow:hidden}}
  .section-header{{background:#f1f3f5;padding:12px 20px;border-bottom:1px solid #dee2e6;font-weight:bold;font-size:1em}}
  table{{width:100%;border-collapse:collapse;font-size:.88em}}
  th{{background:#e9ecef;padding:8px 10px;text-align:left;white-space:nowrap;border-bottom:2px solid #dee2e6}}
  td{{padding:7px 10px;border-bottom:1px solid #f0f0f0;vertical-align:top}}
  tr:hover td{{background:#fafbff}}
  .badge{{display:inline-block;padding:2px 8px;border-radius:12px;font-size:.8em;font-weight:bold;color:#fff}}
  .badge-ok{{background:#28a745}}.badge-warn{{background:#fd7e14}}.badge-bad{{background:#dc3545}}
  .footer{{text-align:center;color:#999;font-size:.8em;padding:12px;margin-top:8px}}
</style>
</head>
<body>
<div class="report-header">
  ...
  <div class="meta">
    ...
    适合量化：{len(stock_results)} 只 &nbsp;|&nbsp;
    不适合：{len(unfit_stocks)} 只
  </div>
</div>

<div class="section">
  <div class="section-header">✅ 适合量化的股票（{len(stock_results)} 只）</div>
  <div style="overflow-x:auto">
  <table>
    <thead>
      <tr>
        <th>代码</th><th>名称</th><th>最优策略</th>
        <th>综合评分</th>
        <th title="回测实际年化收益（与直接回测一致）">实际年化%</th>
        <th title="仓位效率年化 = 实际年化 ÷ 最大仓位比，用于横向比较">仓位效率%</th>
        <th>胜率%</th>
        <th>最大回撤%</th><th>总收益%</th><th>夏普</th><th>交易次数</th>
        <th>加仓建议</th>
      </tr>
    </thead>
    <tbody>{rows_html if rows_html else '<tr><td colspan="12" style="text-align:center;color:#999;padding:20px">无数据</td></tr>'}</tbody>
  </table>
  </div>
</div>

{'<div class="section"><div class="section-header">⚠️ 不适合量化（' + str(len(unfit_stocks)) + ' 只）</div><div style="overflow-x:auto"><table><thead><tr><th>代码</th><th>名称</th><th>原因</th></tr></thead><tbody>' + unfit_rows + '</tbody></table></div></div>' if unfit_stocks else ''}

<div class="footer">⚠️ 免责声明：本报告基于历史回测数据，仅供参考，不构成投资建议。</div>
</body>
</html>"""

    filename = f"best_factor_report_{now_str.replace(' ','_').replace(':','')}.html"
...
@app.route('/api/factors/best/send-wechat', methods=['POST'])
def api_factors_best_send_wechat():
    """将最佳因子报告发送到微信（PushPlus）"""
    data   = request.json or {}
    ...
    lines = [
        ...
        f"**适合量化**：{len(stock_results)} 只　**不适合**：{len(unfit_stocks)} 只",
        f"",
        f"---",
        f"",
        f"### ✅ 适合量化的股票",
    ...
    for entry in stock_results:
        ...
        code   = entry.get('code', '')
        name   = entry.get('name', '')
        strat  = bs.get('strategy_name', '')
        score  = bs.get('composite_score')
        real_a = bs.get('annual_return')           # 实际年化（与直接回测一致）
        adj_a  = bs.get('adj_annual_return')       # 仓位效率年化
        wr     = bs.get('win_rate')
        dd     = bs.get('max_drawdown_pct')
        total  = bs.get('total_return_pct')

        score_icon = '🟢' if (score or 0) >= 75 else ('🟡' if (score or 0) >= 55 else '🔴')
        lines.append(f"#### {score_icon} {name}（{code}）")
        lines.append(f"- **最优策略**：{strat}")
        if real_a is not None and wr is not None and dd is not None and total is not None:
            lines.append(
                f"- 综合评分：**{score if score is not None else '-'}**　"
                f"实际年化：**{real_a:+.1f}%**（仓位效率：{adj_a:+.1f}%）　"
                f"胜率：{wr:.0f}%　回撤：{dd:.1f}%　总收益：{total:+.1f}%"
            )
        else:
            lines.append(f"- 综合评分：{score}")
        if pa and pa.get('summary'):
            lines.append(f"- 加仓建议：{pa['summary']}")
        lines.append("")
    ...
    if unfit_stocks:
        ...
        for u in unfit_stocks:
            lines.append(f"- {u.get('name','')}（{u.get('code','')}）：{u.get('reason','')}")
        lines.append("")

    lines += [
        f"---",
        f"*⚠️ 以上数据基于历史回测，仅供参考，不构成投资建议*",
    ]

    content = "\n".join(lines)
    title   = f"最佳因子报告 {now_str[:10]} ({len(stock_results)}只适合)"

    try:
        results = notification_manager._send(title, content, channels=['wechat'])
        any_ok  = any(v for v in results.values()) if results else False
        if not notification_manager.wechat_enabled:
            return jsonify({'success': False, 'error': '微信推送未配置，请先在系统设置中添加 PushPlus Token'}), 400
        if any_ok:
            return jsonify({'success': True, 'message': f'微信报告已发送（{len(results)} 个账户）'})
        return jsonify({'success': False, 'error': '发送失败，请检查 PushPlus Token 是否有效'}), 500
    except Exception as e:
        logger.error(f"send-wechat error: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
